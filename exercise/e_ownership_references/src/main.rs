...
fn inspect(arg: &str) -> () { // pode usar &str já que não vai ter operação em cima da string
    let is_plural: bool = (*arg).ends_with("s");
    if is_plural {
        println!("plural");
    } else {
        println!("singular");
...
fn eat(arg: String) -> bool {
    if arg.starts_with("b") && arg.contains("a") { true } else { false }
}
